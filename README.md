...
`screen.sh` を使うと、`screen` のセッションとしてアプリを管理できます。

前提条件:

- `screen` がインストールされていること
- `screen.sh` に実行権限があること

Ubuntu で初回セットアップする場合は、次を実行してください。
...
`chmod +x screen.sh` を実行できない場合は、`bash screen.sh start` のように `bash` 経由でも実行できます。
...
- `start`
  - `machine_temperature_reporter` という固定名の screen セッションで `npm start` を起動します
- `stop`
  - 実行中の screen セッションを終了します
- `restart`
  - stop してから start し直します

同じセッションがすでに動いている場合、`start` は何もしません。
